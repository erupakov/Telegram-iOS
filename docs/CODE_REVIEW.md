...
- [ ] **Нет излишних комментариев.** Комментарий объясняет *почему* (неочевидное намерение, обход бага, договорённость с бэком), а не *что* делает код — если строка читается из самого кода, комментарий не нужен.
- [ ] Не дублировать имя метода/переменной словами (`// собираем из непустых сегментов` над очевидной фильтрацией — лишнее).
- [ ] Если комментарий всё же нужен — одна короткая строка, а не абзац. Многострочные пояснения — только для действительно нетривиальной логики (гонки, порядок операций, layout-шторм).
- [ ] Закомментированных «черновиков»/альтернатив быть не должно (см. «Мёртвый код»).
