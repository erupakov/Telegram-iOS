                ...
                if let peer = transaction.getPeer(peerId), let inputPeer = apiInputPeer(peer) {
                    ...
                    if let tag = tag {
                        if tag == MessageTags.unseenPersonalMessage {
                            ...
                            requestSignal = network.request(Api.functions.messages.getUnreadReactions_teamgram_layer201(flags: 0, peer: inputPeer, topMsgId: nil, savedPeerId: nil, offsetId: messageIds[messageIds.count - 1].id + 1, addOffset: 0, limit: Int32(messageIds.count), maxId: messageIds[messageIds.count - 1].id + 1, minId: messageIds[0].id - 1))
