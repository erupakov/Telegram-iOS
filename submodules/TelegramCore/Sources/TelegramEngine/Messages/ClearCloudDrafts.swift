                ...
                case let .updates(updatesData):
                    ...
                    for key in keys {
                        ...
                        if let peer = transaction.getPeer(key.peerId), let inputPeer = apiInputPeer(peer) {
                            ...
                            signals.append(network.request(Api.functions.messages.saveDraft_teamgram_layer201(flags: flags, replyTo: replyTo, peer: inputPeer, message: "", entities: nil, media: nil, effect: nil))
