    ...
    private func actionPressed() {
        ...
        switch wallpaper {
            case .image:
                ...
                |> filter { status -> Bool in
                    ...
                    if case let .uploaded(uploadedWallpaper, resultWallpaper) = status, uploadedWallpaper == wallpaper, case let .file(file) = resultWallpaper {
                        ...
                        let shareController = ShareController(context: context, subject: .url("https://t.divo.global/bg/\(file.slug)\(optionsString)"))
            ...
            case let .file(file):
                ...
                controller = ShareController(context: context, subject: .url("https://t.divo.global/bg/\(file.slug)\(optionsString)"))
            case let .color(color):
                controller = ShareController(context: context, subject: .url("https://t.divo.global/bg/\(UIColor(rgb: color).hexString)"))
            case let .gradient(gradient):
                var colorsString = ""
                ...
                controller = ShareController(context: context, subject: .url("https://t.divo.global/bg/\(colorsString)"))
            default:
                break
        }
