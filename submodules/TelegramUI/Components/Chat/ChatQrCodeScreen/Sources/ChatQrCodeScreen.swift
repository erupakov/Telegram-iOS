...
private class QrContentNode: ASDisplayNode, ContentNode {
    ...
    init(context: AccountContext, peer: Peer, threadId: Int64?, isStatic: Bool = false, temporary: Bool) {
        ...
        var codeLink: String
        if let addressName = peer.addressName, !addressName.isEmpty {
            codeLink = "https://t.divo.global/\(peer.addressName ?? "")"
        } else if let peer = peer as? TelegramUser {
            codeLink = "https://t.divo.global/+\(peer.phone ?? "")"
        } else if let _ = peer as? TelegramChannel {
            codeLink = "https://t.divo.global/c/\(peer.id.id._internalGetInt64Value())"
        } else {
            codeLink = ""
        }
        ...
        if temporary {
            ...
            self.codeMarkersNode.setSignal(qrCode(string: "https://t.divo.global/contact/000000:abcdef", color: .black, backgroundColor: nil, icon: .cutout, ecl: "Q", onlyMarkers: true) |> map { $0.1 }, attemptSynchronously: true)
        }
    }
...
private class MessageContentNode: ASDisplayNode, ContentNode {
    ...
    func updateLayout(size: CGSize, topInset: CGFloat, bottomInset: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        if let message = self.messages.first, let chatPeer = message.peers[message.id.peerId] as? TelegramChannel, message.id.namespace == Namespaces.Message.Cloud, let addressName = chatPeer.addressName, !addressName.isEmpty {
            link = "t.divo.global/\(addressName)/\(message.id.id)"
        } else {
            link = ""
        }
